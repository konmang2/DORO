  const data = [
  {
    "category": "여행자",
    "type": "객관식",
    "question": "라이덴 메이의 원소 폭발 사용시 나오는 대사가 아닌 것은 무엇인가요?",
    "options": ["무념 단절","지금이 적멸의 시간","뇌광이 애처롭구나","번개는 영원히 빛나리"],
    "answer": [2],
    "timer": 40
  },
  {
    "category": "여행자",
    "type": "주관식",
    "question": "티바트 외부에서 온 존재들로, 주인공인 여행자도 포함되는 이들의 명칭은 무엇인가요?",
    "answerText": "강림자",
    "timer": 40
  },
  {
    "category": "여행자",
    "type": "이미지",
    "question": "다음 실루엣은 무엇일까요?",
    "image": "https://github.com/konmang2/quiz-image/blob/main/%ED%96%A5%EB%A6%892%EC%9B%90%EB%B3%B822.png?raw=true",
    "answerText": "누룽지",
    "revealImage": "https://github.com/konmang2/quiz-image/blob/main/%ED%96%A5%EB%A6%892%EC%9B%90%EB%B3%B82.png?raw=true",
    "timer": 40
  },
  {
    "category": "여행자",
    "type": "다중정답+주관식",
    "question": "나히다의 원소폭발 「마음이 그리는 환상」은 사용시 파티 내 특정 원소 타입의 캐릭터가 있으면 버프를 받는데, 이 특정 원소는 무엇인가요?",
    "answer": ["물","불","번개"],
    "timer": 40,
    "explanation": "불 - 원소스킬 피해증가\n물 - 궁극 지속시간 증가\n번개 - 원소스킬 발동간격 감소"
  },
  {
    "category": "여행자",
    "type": "순서",
    "question": "다음 우인단들의 서열을 높은 순서대로 이동해주세요.",
    "options": [
      "https://github.com/konmang2/quiz-image/blob/main/%EC%82%B0%EB%93%9C%EB%A1%9C%EB%84%A4.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%95%84%EB%A5%BC%EB%A0%88%ED%82%A4%EB%85%B8.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%B9%B4%ED%94%BC%ED%83%80%EB%85%B8.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%BD%9C%EB%A1%AC%EB%B9%84%EB%82%98.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%ED%83%80%EB%A5%B4%ED%83%88%EB%A6%AC%EC%95%84.png?raw=true"
    ],
    "answerOrder": [2,3,1,0,4],
    "timer": 40,
    "explanation": "카피타노 - 1위\n콜롬비나 - 3위\n아를레키노 - 4위\n산드로네 - 7위\n타르탈리아 - 11위"
  },
  {
    "category": "개척자",
    "type": "빈칸",
    "question": "____, 기나긴 꿈!",
    "audio": "https://github.com/konmang2/quiz-image/blob/main/%EB%8B%A8%ED%95%AD.wav",
    "answerText": "동천환화",
    "timer": 40,
    "explanation": "단항의 궁극기 대사입니다."
  },
  {
    "category": "개척자",
    "type": "순서",
    "question": "다음 캐릭터들의 현상금을 낮은 순으로 나열해주세요.",
    "options": [
      "https://github.com/konmang2/quiz-image/blob/main/%EB%B0%98%EB%94%94.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EB%B8%94%EB%A0%88%EC%9D%B4%EB%93%9C.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%9D%80%EB%9E%91.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%B9%B4%ED%94%84%EC%B9%B4.png?raw=true"
    ],
    "answerOrder": [2,1,0,3],
    "timer": 40,
    "explanation": "은랑 - 51억\n블레이드 - 81억 3000만\n반디 - 97억 2300만\n카프카 - 108억 9900만"
  },
  {
    "category": "개척자",
    "type": "다중정답+주관식",
    "question": "엠포리어스 기준, 개척자가 사용할 수 있는 모든 속성과 운명의 길을 적어주세요.",
    "answer": ["물리","화염","허수","얼음","파멸","보존","화합","기억"],
    "timer": 60,
    "explanation": "헤르타 - 물리·파멸\n야릴로-Ⅵ - 화염·보존\n페나코니 - 허수·화합\n엠포리어스 - 얼음·기억"
  },
  {
    "category": "개척자",
    "type": "주관식",
    "question": "행동게이지 관련 스킬이 있고, 무기로 총을 사용하며, 현상금이 있습니다. 저는 누구일까요?",
    "answerText": "부트힐",
    "timer": 40
  },
  {
    "category": "개척자",
    "type": "이미지",
    "question": "다음 실루엣은 무엇인가요?",
    "image": "https://github.com/konmang2/quiz-image/blob/main/%EB%9D%BC%EC%9D%B4%EB%8D%B41_%EC%8B%A4%EB%A3%A8%EC%97%A3.png?raw=true",
    "answerText": "아케론",
    "revealImage": "https://github.com/konmang2/quiz-image/blob/main/%EB%9D%BC%EC%9D%B4%EB%8D%B41.png?raw=true",
    "timer": 40
  },
  {
    "category": "로프꾼",
    "type": "객관식",
    "question": "다음 캐릭터들 중에서 고양이 시렌이 아닌 캐릭터를 골라주세요.",
    "options": ["세스","네코마타","펄크라","미야비"],
    "answer": [3],
    "timer": 40,
    "explanation": "미야비는 여우 수인입니다."
  },
  {
    "category": "로프꾼",
    "type": "객관식",
    "question": "다음 중 진영과 전용 방부 연결이 틀린 것은 무엇인가요?",
    "options": ["교활한 토끼굴 - 에밀리온","스타즈 오브 리라 - 찰칵이","빅토리아 하우스 키핑 - 버틀러","형사 특수팀 - 걸리버 요원"],
    "answer": [3],
    "timer": 40,
    "explanation": "형사 특수팀의 방부는 걸리버 요원이 아닌 최 순찰대원입니다."
  },
  {
    "category": "로프꾼",
    "type": "다중정답+주관식",
    "question": "칼리돈의 자손 중, 플레이어블 캐릭터로 등장하는 인물들을 모두 적어주세요. (1.7 버전 기준)",
    "answer": ["루시","파이퍼","카이사르","버니스","라이터","펄크라"],
    "timer": 60,
    "explanation": "펄크라는 1.6버전부터 합류했습니다."
  },
  {
    "category": "로프꾼",
    "type": "주관식",
    "question": "미야비는 시스템상 얼음 속성으로 분류되지만 실제로는 고유한 속성을 지니고 있습니다. 이 속성의 이름은 무엇인가요?",
    "answerText": "서리",
    "timer": 40
  },
  {
    "category": "로프꾼",
    "type": "이미지",
    "question": "다음 실루엣은 무엇일까요?",
    "image": "https://github.com/konmang2/quiz-image/blob/main/%EC%B2%AD%EC%9D%98_%EC%8B%A4%EB%A3%A8%EC%97%A3.png?raw=true",
    "answerText": "청의",
    "revealImage": "https://github.com/konmang2/quiz-image/blob/main/%EC%B2%AD%EC%9D%98.png?raw=true",
    "timer": 40
  },
  {
    "category": "방랑자",
    "type": "주관식",
    "question": "명조의 초반 스토리에 계속 등장하는 것으로, 엄청난 철학이 담겨져 있어 아직 그 뜻을 이해하지 못한 사람들이 많은 이것은?",
    "answerText": "망고스틴",
    "timer": 40,
    "explanation": "아직도 그 의미를 모르는 사람이 많습니다."
  },
  {
    "category": "방랑자",
    "type": "다중정답+주관식",
    "question": "명조에 존재하는 모든 속성을 적어주세요.",
    "answer": ["응결","용융","전도","기류","회절","인멸"],
    "timer": 60
  },
  {
    "category": "방랑자",
    "type": "객관식",
    "question": "명조의 속성 중 유일하게 5성 캐릭터로만 이루어진 속성은 무엇인가요? (2.3 버전 기준)",
    "options": ["응결","기류","인멸","회절"],
    "answer": [3],
    "timer": 40,
    "explanation": "회절 속성은 방랑자, 벨리나, 금희, 파수인, 페비, 젠니만 보유합니다."
  },
  {
    "category": "방랑자",
    "type": "객관식",
    "question": "다음 캐릭터 중 무기 유형이 잘못 연결된 것은 무엇인가요?",
    "options": ["금희-대검","카멜리아-직검","젠니-권총","칸타렐라-증폭기"],
    "answer": [2],
    "timer": 40,
    "explanation": "젠니의 무기 유형은 권갑입니다."
  },
  {
    "category": "방랑자",
    "type": "이미지",
    "question": "다음 실루엣은 무엇인가요?",
    "image": "https://github.com/konmang2/quiz-image/blob/main/%EC%B9%B4%EB%A9%9C_%EC%8B%A4%EB%A3%A8%EC%97%A3_%EA%B0%9C%EC%84%A0.png?raw=true",
    "answerText": "카멜리아",
    "revealImage": "https://github.com/konmang2/quiz-image/blob/main/%EC%B9%B4%EB%A9%9C.jpg?raw=true",
    "timer": 40
  },
  {
    "category": "단테",
    "type": "주관식",
    "question": "Project Moon의 이전 작품에 등장한 인물과 환상체의 E.G.O 및 인격들을 한정으로 등장시키는 추출 이벤트의 명칭은 무엇인가요?",
    "answerText": "발푸르기스의 밤",
    "timer": 60
  },
  {
    "category": "단테",
    "type": "주관식",
    "question": "이상의 E.G.O 기본 명칭은 무엇인가요?",
    "answerText": "오감도",
    "timer": 40
  },
  {
    "category": "단테",
    "type": "객관식",
    "question": "돈키호테의 인격 중 출혈을 메인으로 사용하는 인격은 무엇인가요?",
    "options": [
      "[검계 살수] 돈키호테",
      "[남부 셩크 협회 5과 부장] 돈키호테",
      "[W사 3등급 정리 요원] 돈키호테",
      "[라만차랜드 실장] 돈키호테"
    ],
    "answer": [3],
    "timer": 60,
    "explanation": "[검계 살수] 돈키호테 – 호흡\n[남부 셩크 협회 5과 부장] 돈키호테 – 속도\n[W사 3등급 정리 요원] 돈키호테 – 충전"
  },
  {
    "category": "단테",
    "type": "이미지",
    "question": "다음 실루엣은 무엇일까요?",
    "image": "https://github.com/konmang2/quiz-image/blob/main/%E3%84%BC%E3%85%851A.png?raw=true",
    "answerText": "와일드헌트 히스클리프",
    "revealImage": "https://github.com/konmang2/quiz-image/blob/main/%E3%84%BC%E3%85%851.PNG?raw=true",
    "timer": 60
  },
  {
    "category": "단테",
    "type": "주관식",
    "question": "료슈의 기본 E.G.O ‘삼라염상’의 대사 ‘모.불.아.위’의 뜻은 무엇인가요?",
    "answerText": "모든 것을 불살라서라도 아름다움을 위해",
    "timer": 60
  },
  {
    "category": "지휘관(니케)",
    "type": "주관식",
    "question": "방주 소속이 아닌 니케들은 무엇이라고 부르나요?",
    "answerText": "필그림",
    "timer": 60
  },
  {
    "category": "지휘관(니케)",
    "type": "객관식",
    "question": "다음 중 소속 그룹과 멤버가 잘못 연결된 것은 무엇인가요?",
    "options": [
      "테트라 – 앨리스, 블랑, 아니스, 프리바티",
      "필그림 – 홍련, 도로시, 크라운, 리틀 머메이드",
      "미실리스 – 리타, 라플라스, 일레그, 마나",
      "엘리시온 – 프리바티, 길로틴, 라피, D"
    ],
    "answer": [0],
    "timer": 60,
    "explanation": "프리바티는 테트라 소속이 아닌 엘리시온 소속입니다."
  },
  {
    "category": "지휘관(니케)",
    "type": "다중정답+주관식",
    "question": "갓데스 스쿼드 멤버를 전부 적어주세요.",
    "answer": ["릴리바이스","도로시","레드후드","스노우화이트","홍련","라푼젤"],
    "timer": 60
  },
  {
    "category": "지휘관(니케)",
    "type": "객관식",
    "question": "다음 중 니케와 콜라보하지 않은 게임은 무엇인가요?",
    "options": ["체인소맨","에반게리온 신극장판","데이브 더 다이버","주술회전"],
    "answer": [3],
    "timer": 60
  },
  {
    "category": "지휘관(니케)",
    "type": "순서",
    "question": "다음 캐릭터들을 버스트 단계 낮은 순서대로 나열해주세요. (1 → 2 → 3 → Λ)",
    "options": [
      "https://github.com/konmang2/quiz-image/blob/main/%EB%A0%88%EB%93%9C%ED%9B%84%EB%93%9C.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%95%84%EC%8A%A4%EC%B9%B4.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%95%A0%EB%93%9C%EB%AF%B8.png?raw=true",
      "https://github.com/konmang2/quiz-image/blob/main/%EC%97%91%EC%8B%9C%EC%95%84.png?raw=true"
    ],
    "answerOrder": [3,2,1,0],
    "timer": 40,
    "explanation": "엑시아 – 1단계\n애드미 – 2단계\n아스카 – 3단계\n레드후드 – Λ단계"
  }
];
